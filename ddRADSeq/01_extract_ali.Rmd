...
```{r, echo=FALSE}
(extr <- tibble(
  liftons_ul = (80*params$num_sample)*1.1,
  enzyme_ul = (40*params$num_sample)*1.1,
  tips_100 = 8*params$num_sample,
  plates = 2*ceiling(params$num_sample/96),
  prot_K_ul = (40*params$num_sample)*1.1,
  seals = plates,
  hybr_buffer = (45*params$num_sample)*1.1,
  ampure_ul = (15*params$num_sample)*1.1,
  tips_300 = 6*params$num_sample,
  ethanol_ul = 3*(150*params$num_sample)*1.1,
  low_te = (45*params$num_sample)*1.1) %>% 
...
liftons <- tibble(ingredient = c("mMEDTA", "mM_Tris-HCl_pH7.5", "perc_SDS", "pH2O"), initial_conc = c(500, 1000, 20, NA),final_conc = c(100, 25, 1, NA), final_vol_mL = extr_wide$liftons_ul/1000) %>% 
  mutate(initial_vol_mL = final_conc* final_vol_mL/initial_conc,
