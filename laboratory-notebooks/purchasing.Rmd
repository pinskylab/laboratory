...
####Extract 
```{r}
rmarkdown::render("~/Documents/laboratory_pinskylab/protocols/01_extract_ali.Rmd", params = params)
rm(enzyme, extr_wide, hybrid, lab, leyte, liftons, params, plate, samples, work, low_te)
```
#### Quant
```{r}
rmarkdown::render(here::here("protocols", "02_quant_extract.Rmd"), params = params)
```
...
grocery <- grocery_list %>% 
  ...
  mutate(total_cost = cost_per_item * quantity, 
         cost_per_sample = format(total_cost/params$num_sample, nsmall = 2, scientific = F))
