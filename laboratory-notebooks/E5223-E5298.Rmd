...
## Special notes:  
**There is no APCL18_703 tube in the box.  Replaced in extraction database with XXXX.**  


**In the 12th column of the plate, the second halves of some fin clips were used to test the Ali extraction method.  These samples had been previously extracted in Qiagen, so we could see the difference in extraction performance.** 


**75uL of crude lysate per sample is still in the fridge. All processed extract went into digest.**
...
3. Add 45uL of the lysate and pipet up and down to mix.  The rest of crude lysate (75uL) is in the fridge.
...
## Special fix, because there is no APCL18_703
plate <- plate %>% 
  mutate(sample_id = ifelse(sample_id == "APCL18_703", "XXXX", sample_id))
```
...
#     final_vol = "45")
# 
# plate_name <- extr_plate %>% 
#   summarise(first = min(extraction_id), 
#     last = max(extraction_id, na.rm = T))
# 
# extr_plate <- extr_plate %>% 
#   mutate(plate = paste(plate_name$first, plate_name$last, sep = "-"))

# To generate plate map if extract_ids have already been assigned
extr_plate <- lab %>% tbl("extraction") %>%  collect() %>% 
  filter(sample_id %in% !!plate$sample_id & sample_id != "XXXX") 

extr_plate <- lab %>% tbl("extraction") %>%  collect() %>% 
  filter(plate %in% extr_plate$plate) %>% 
  arrange(sample_id) %>%
...
extr_plate <- select(extr_plate, -row, -col)
...
# dbWriteTable(lab, "extraction", extr_plate, row.names = F, overwrite = F, append = T)
