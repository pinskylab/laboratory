...
```{r, echo=FALSE}
(quant <- tibble(
  lambda_DNA_ul = (20)*1.1,
  TE_1x_ul = ((980)*1.1)+((params$num_samples + (params$num_plates*8))*1.1*200/2/1000)+2989,
  tips_10 = 2 + params$num_samples, 
  tips_100 = 1 + (params$num_samples + 8)*2,
  tips_1000 = 4,
  tubes = 5,
  gren_plates = ceiling(params$num_samples/96) + 1,
  foil_seals = 1, 
  pico_ul = (params$num_samples + (params$num_plates*8))*1.1/2) %>% gather(item, quantity) %>% 
...
```{r echo=FALSE}
final_vol <- (params$num_samples + (params$num_plates*8))*1.1*200/2
```
...
1. Add 100ul pico dilution to every well that contains either a standard of a sample.  
...
Once you have completed all of the steps in the quant_prep.pdf,  make a note for your lab notebook
