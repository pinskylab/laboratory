...
params:
  first_plate: E6432-E6496
  last_plate: E6432-E6496
  id: extraction_id
  num_plates: 1
  num_sample: 5
  type: extraction
...
(quant <- tibble(
  ...
  TE_1x_ul = ((980)*1.1)+((params$num_sample + (params$num_plates*8))*1.1*200/2/1000)+2989,
  tips_10 = 2 + params$num_sample, 
  tips_100 = 1 + (params$num_sample + 8)*2,
  tips_1000 = 4,
  tubes = 5,
  gren_plates = ceiling(params$num_sample/96) + 1,
  foil_seals = 1, 
  pico_ul = (params$num_sample + (params$num_plates*8))*1.1/2) %>% gather(item, quantity) %>% 
...
```{r reader_results, echo=FALSE, message=FALSE, eval=FALSE}
...
```{r sample_names, echo=FALSE, eval=FALSE}
